...
import { EyeTrackerProvider, useEyeTracker } from '../../contexts/EyeTrackerContext'
...
// Inner component that uses the auth sync hook and eye tracker
const LayoutContent = () => {
  // Sync auth to storage for content script
  useAuthSync();
  
  // Get eye tracker status for global display
  const { deviceStatus } = useEyeTracker();
      ...
      case DeviceStatus.CONNECTED:
        return (
          <div className="plasmo-flex plasmo-items-center plasmo-gap-1">
            <div className="plasmo-w-2 plasmo-h-2 plasmo-bg-green-400 plasmo-rounded-full plasmo-animate-pulse"></div>
            <span className="plasmo-text-xs plasmo-text-green-300">Connected</span>
          </div>
        )
      case DeviceStatus.CONNECTING:
        return (
          <div className="plasmo-flex plasmo-items-center plasmo-gap-1">
            <div className="plasmo-w-2 plasmo-h-2 plasmo-bg-yellow-400 plasmo-rounded-full plasmo-animate-pulse"></div>
            <span className="plasmo-text-xs plasmo-text-yellow-300">Connecting</span>
          </div>
        )
      case DeviceStatus.ERROR:
        return (
          <div className="plasmo-flex plasmo-items-center plasmo-gap-1">
            <div className="plasmo-w-2 plasmo-h-2 plasmo-bg-red-400 plasmo-rounded-full"></div>
            <span className="plasmo-text-xs plasmo-text-red-300">Error</span>
          </div>
        )
      default:
        return null
  ...
  return (
    <div className="plasmo-w-[380px] plasmo-h-[600px] plasmo-bg-white plasmo-flex plasmo-flex-col plasmo-overflow-hidden">
      ...
      <footer className="plasmo-bg-white plasmo-border-t plasmo-border-gray-200 plasmo-px-4 plasmo-py-3">
        ...
        <SignedOut>
          <nav className="plasmo-flex plasmo-justify-center plasmo-gap-4">
            <Link 
              to="/sign-in" 
              className="plasmo-px-4 plasmo-py-2 plasmo-text-sm plasmo-font-medium plasmo-text-indigo-600 hover:plasmo-text-indigo-700 plasmo-transition-colors"
            >
              Sign In
            </Link>
            <Link 
              to="/sign-up" 
              className="plasmo-px-4 plasmo-py-2 plasmo-text-sm plasmo-font-medium plasmo-bg-indigo-600 plasmo-text-white plasmo-rounded-md hover:plasmo-bg-indigo-700 plasmo-transition-colors"
            >
              Sign Up
            </Link>
          </nav>
